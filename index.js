...
var input, plays, die1, die2, dieSum, balance, banner, outcome, die1Name, die2Name, money, negative, numRolls, text; // define variables
balance = 0;  // initial value
var die1Image = new Image();
var die2Image = new Image();

function playNum() {
  input = prompt("How many times would you like to roll?");
  plays = Number(input);
}
...
function whoWon() {
    ...
    if (balance < 0) {
      text = "" + balance;
      money = "Your balance is now " + text.replace("-", "-$");
    } 
    else {
      money = "Your balance is now $" + balance;
    }
    document.querySelector("h3").innerHTML = banner + "<br>" + outcome + "<br>" + money;
}
